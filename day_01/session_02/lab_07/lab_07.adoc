...
In this lab, we'll use a Spring Boot CLI app designed to illustrate Cloud Foundry operations such as scaling. In the case your PCF instance does not have internet access you could prebuilt the app local as a jar and push it afterwards. You could also use any other application you like for this lab (for example spring-music).
--


== (Optional) Prebuilt the Application if your PCF instance does not have internet access
...
. Prebuilt the Application _if your PCF instance does not have internet access_ (Install spring boot cli: http://docs.spring.io/autorepo/docs/spring-boot/1.1.4.RELEASE/reference/html/getting-started-installing-spring-boot.html#getting-started-installing-the-cli):
+
----
$ spring jar cf-scale-boot.jar app.groovy
----

. Push the application:
+
----
$ cf push -p ./cf-scale-boot.jar
----
