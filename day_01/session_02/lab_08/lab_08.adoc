...
. Choose `I use Heroku` (now stating Cloud Foundry as well) and provide a name:
+
...
Pivotal Cloud Foundry provides an own metrics service (to be installed on OpsManager) that provides you with the basic metric info that flows in on the second. More info can be found at http://docs.pivotal.io/pcf-metrics/using.html

If you're running your application on Pivotal Web Services check the link:metrics.run.pivotal.io[Metrics] for your App!
